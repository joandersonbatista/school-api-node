...
const StudentCreateValidations_1 = require("../../createStudent/validations/StudentCreateValidations");
class StudentUpdateValidations {
    ...
    heightValidation(student) {
        if (student.height === undefined)
            return;
        if (!(0, StudentCreateValidations_1.isFloat)(student.height))
            throw new Error("The height has to be float type");
    }
    weightValidation(student) {
        if (student.weight === undefined)
            return;
        if (!(0, StudentCreateValidations_1.isFloat)(student.weight))
